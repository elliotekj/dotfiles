...
if [[ -n "$TMUX_PANE" ]]; then
  ...
  my_pid=$$
  target_pid=$my_pid
  pane_list=$(tmux list-panes -a -F '#{pane_pid} #{session_name} #{window_index} #{pane_id}' 2>/dev/null) || exit 0

  session=""
  window=""
  pane_id=""
  while [[ -n "$target_pid" && "$target_pid" != "0" && "$target_pid" != "1" ]]; do
    match=$(echo "$pane_list" | awk -v pid="$target_pid" '$1 == pid { print $2, $3, $4; exit }')
    if [[ -n "$match" ]]; then
      session=$(echo "$match" | cut -d' ' -f1)
      window=$(echo "$match" | cut -d' ' -f2)
      pane_id=$(echo "$match" | cut -d' ' -f3)
      pane_id="${pane_id#%}"
      break
    fi
  ...
  [[ -z "$session" || -z "$window" || -z "$pane_id" ]] && exit 0
fi

cache_file="$CACHE_DIR/$session/$window/$pane_id"

if [[ "$hook_event" == "SessionEnd" ]]; then
  rm -f "$cache_file"
  rmdir "$CACHE_DIR/$session/$window" 2>/dev/null || true
  rmdir "$CACHE_DIR/$session" 2>/dev/null || true
  exit 0
fi
...
mkdir -p "$CACHE_DIR/$session/$window"
